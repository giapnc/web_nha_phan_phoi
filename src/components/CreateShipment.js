...
const CreateShipment = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!ownerAddress) {
          setErrors({
            general:
              "Không tìm thấy địa chỉ ví nhà phân phối. Vui lòng đăng nhập.",
          })
          return
        }
        ...
        const inventoryResponse =
          await distributorService.getInventoryByWallet(ownerAddress)
        ...
        if (inventoryResponse.success && inventoryResponse.data) {
          ...
          const availableBatches = inventoryResponse.data
            .filter(batch => {
              ...
              console.log(
                `🔍 Batch ${batch.batchId}: quantity=${batch.quantity}, hasQuantity=${hasQuantity}, isExpired=${isExpired}`,
              )
            ...
            .map(batch => {
              ...
              if (!blockchainBatchId) {
                console.error(
                  "❌ CRITICAL: Batch missing blockchain batch ID!",
                  batch,
                )
              }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        toast.success(
          `Shipment đã được tạo thành công! Mã shipment: ${response.data?.shipmentCode || "N/A"}`,
        )
      ...
      toast.error(
        "Có lỗi xảy ra khi tạo shipment: " +
              ...
              <div className="summary-section">
                ...
                <div className="summary-card">
                  ...
                  <div
                    ...
                    <span className="blockchain-id">
                      {selectedBatch?.batchId || selectedBatch?.id}
                    </span>
                  </div>
