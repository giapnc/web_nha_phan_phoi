...
const ReceiveGoods = () => {
  ...
  const fetchPendingShipments = async () => {
    try {
      ...
      const response =
        await distributorService.getInboundShipments(recipientAddress)
      ...
      if (response && response.success && Array.isArray(response.data)) {
        ...
        const validShipments = response.data.filter(
          shipment => shipment && (shipment.shipmentId || shipment.id),
        )
        const normalizedShipments = validShipments.map(shipment =>
  ...
  const handleScan = async () => {
    if (!scanInput.trim()) {
      setError(
        "Vui lòng nhập mã shipment (SHIP-xxxxxxxxxx hoặc số shipment ID)",
      )
      return
    }
    ...
    try {
      ...
      const foundShipment = (pendingShipments || []).find(
        shipment =>
          shipment.trackingCode === scanInput ||
      ...
      if (foundShipment) {
        ...
        try {
          ...
          if (verification.success && verification.data.isOwner) {
            ...
            setError(
              "Lô hàng này chưa được chuyển quyền sở hữu trên blockchain. Có thể là hàng giả!",
            )
            return
          }
        } catch (e) {
      ...
      try {
        ...
        if (byId.success && byId.data) {
          ...
          const recipientAddress = localStorage.getItem("walletAddress")
          if (
            byId.data.toAddress?.toLowerCase() ===
            ...
            if (byId.data.batchId) {
              try {
                const batchRes = await distributorService.getBatchDetails(
                  byId.data.batchId,
                )
            ...
            const normalized = {
              ...
              trackingCode:
                byId.data.shipmentCode || `SHIP-${byId.data.shipmentId}`,
              ...
              expectedDate:
                byId.data.expectedDeliveryDate || byId.data.shipmentTimestamp,
      ...
      try {
        let shipmentsRes =
          await distributorService.getShipmentsByBatch(scanInput)
        let shipments = Array.isArray(shipmentsRes?.data)
          ? shipmentsRes.data
          : []
        ...
        if (!shipments.length) {
          ...
          const foundBatch = (allBatches?.data || []).find(
            b =>
              b.batchNumber === scanInput ||
              b.id?.toString?.() === scanInput ||
              b.batchId?.toString?.() === scanInput,
          )
          if (foundBatch) {
            const batchId =
              foundBatch.batchId?.toString?.() ||
              foundBatch.id?.toString?.() ||
              foundBatch.batchNumber
            shipmentsRes = await distributorService.getShipmentsByBatch(batchId)
            shipments = Array.isArray(shipmentsRes?.data)
              ? shipmentsRes.data
              : []
          }
        }
        ...
        if (shipments.length) {
          const recipientAddress = localStorage
            .getItem("walletAddress")
            ?.toLowerCase?.()
          const pendingForMe = shipments.find(
            s =>
              (s.status?.toLowerCase?.() === "pending" ||
                s.status?.toLowerCase?.() === "in_transit") &&
              (s.recipientAddress?.toLowerCase?.() === recipientAddress ||
                s.toAddress?.toLowerCase?.() === recipientAddress),
          )
          if (pendingForMe) {
            ...
            const normalized = {
              ...
              trackingCode:
                pendingForMe.shipmentCode ||
                pendingForMe.trackingCode ||
                `SHIP-${pendingForMe.shipmentId}` ||
                pendingForMe.id,
              from:
                pendingForMe.senderName || pendingForMe.from || "Nhà sản xuất",
              expectedDate:
                pendingForMe.estimatedDelivery || pendingForMe.createdAt,
        ...
        setError(
          `Không tìm thấy shipment với mã: ${scanInput}. Vui lòng kiểm tra lại mã shipment (định dạng: SHIP-xxxxxxxxxx hoặc số shipment ID).`,
        )
      } catch (err) {
        console.warn("Could not find shipments by batch:", err)
        setError(
          `Không tìm thấy shipment với mã: ${scanInput}. Vui lòng kiểm tra lại mã shipment.`,
        )
      }
    } catch (err) {
  ...
  const handleConfirmReceive = async () => {
    ...
    try {
      ...
      // Strategy 2: Use shipmentId from blockchain
        ...
        console.log(
          "Using blockchain shipmentId for API call:",
          shipmentIdForApi,
        )
      }
      ...
      const response =
        await distributorService.receiveShipment(shipmentIdForApi)
      ...
      if (response.success) {
        ...
        setSuccess(
          `Đã xác nhận nhận hàng thành công! Giao dịch blockchain (nhận hàng): ${txHash}`,
        )
      ...
      <div className="scanner-section">
        <div className="scanner-card">
          <div className="scanner-header">
            ...
            <p>
              Quét mã shipment từ phiếu giao hàng hoặc nhập trực tiếp mã
              SHIP-xxxxxxxxxx
            </p>
          </div>
                ...
                <div className="info-grid">
                  ...
                  <div className="info-item">
                    ...
                    <span className="value">
                      {shipmentDetails.trackingCode}
                    </span>
                  </div>
                  <div className="info-item">
                    <span className="label">Ngày dự kiến:</span>
                    <span className="value">
                      {formatDate(shipmentDetails.expectedDate)}
                    </span>
                  </div>
                  <div className="info-item">
                    <span className="label">Tổng giá trị:</span>
                    <span className="value highlight">
                      {formatCurrency(shipmentDetails.totalValue)}
                    </span>
                  </div>
              ...
              <div className="products-section">
                ...
                <div className="products-table">
                  <table>
                    ...
                    <tbody>
                      {(shipmentDetails.products || []).map(
                        (product, index) => (
                          <tr key={index}>
                            <td className="product-name">{product.name}</td>
                            <td className="batch-code">{product.batchCode}</td>
                            <td className="quantity">
                              {product.quantity?.toLocaleString() || 0} hộp
                            </td>
                            <td className="expiry">
                              {product.expiry || "N/A"}
                            </td>
                            <td className="manufacturer">
                              {product.manufacturer || shipmentDetails.from}
                            </td>
                          </tr>
                        ),
                      )}
                      {(!shipmentDetails.products ||
